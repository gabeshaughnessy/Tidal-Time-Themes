...
// SanFrancisco Latitude and Longitude value="37.77:North,122.42:West"
var suncalc_updated
...
SiteLat=37.77; //sf lat
LatDir="North";//sf direction
SiteLon=122.42;//sf long
LonDir="West";//sf direction
...
pos.MoonLon = output_moon_angles[3];
pos.MoonLat = output_moon_angles[11];
pos.MoonRa = output_moon_angles[4];
pos.MoonDecl = output_moon_angles[2];
pos.MoonAz = output_moon_angles[1];
pos.MoonDistance = output_moon_angles[13]*6378.14;
pos.MoonGeoEl = output_moon_angles[10];
pos.MoonEl = output_moon_angles[12];

console.log("Moon Longitude: " + output_moon_angles[3]);
console.log("Moon Latitude: " + output_moon_angles[11]);
console.log("Moon Right Ascension: " + output_moon_angles[4]);
console.log("Moon Declination: " + output_moon_angles[2]);
console.log("Moon Azimuth: " + output_moon_angles[1]);
console.log("Moon Distance (km): " + output_moon_angles[13]*6378.14);
console.log("Moon Geometric Elevation(with atmospheric refraction): " + output_moon_angles[10]);//atmospheric refraction appearance
console.log("Moon Elevation: " + output_moon_angles[12]);//without atmospheric refraction


if (pos.MoonGeoEl >-5) 
  pos.MoonEl=formatvalue(pos.MoonEl,5);
  else pos.MoonEl='N/A';//formatvalue(SunAltitude,5);


pos.SunAltitude=output_angles[0];
...
pos.SunDeclination=output_angles[2];
pos.SunLongitude=output_angles[3];
pos.RA=output_angles[4];
pos.GMST0=output_angles[5];
pos.MeanAnomaly=output_angles[6];
pos.Perihelion=output_angles[7];
pos.Eccentricity=output_angles[8];
pos.Obliquity=output_angles[9];
pos.SunGeometricElevation=output_angles[10];

console.log('SunAltitude: ' + output_angles[0]);
console.log( 'SunDeclination:' + output_angles[2]);
console.log( 'SunLongitude:' +output_angles[3]);
console.log( 'RA:' +output_angles[4]);
console.log( 'GMST0:' +output_angles[5]);
console.log( 'MeanAnomaly:' +output_angles[6]);
console.log( 'Perihelion:' +output_angles[7]);
console.log( 'Eccentricity:' +output_angles[8]);
console.log( 'Obliquity:' +output_angles[9]);
console.log( 'SunGeometricElevation:' +output_angles[10]);

if (pos.SunGeometricElevation>-5) 
  pos.SunElevation=formatvalue(pos.SunAltitude,5);
  else pos.SunElevation='N/A';//formatvalue(SunAltitude,5);



pos.SatelliteLongitude=getsatelliteposition(output_angles[1]);  // input azimuth


pos.SunSatLonArray=getsatelliteposition(output_angles[1]);
pos.SatelliteLongitude=pos.SunSatLonArray[0];


pos.SunSatLon=pos.SunSatLonArray[1]*1;
//alert(SunSatLon);

if (pos.SatelliteLongitude=="Not Available") 
  {
  pos.SatelliteElevation="N/A";
  pos.ElevationDifference="N/A";
//ElevationDifference=Math.abs(document.Angles.SunGeometricElevation.value-document.Angles.SatelliteElevation.value);
   }
   else 
   {
   pos.SatelliteElevation=formatvalue(Elevation2(pos.SunSatLon,SiteLat,SiteLon,0),5);
   pos.ElevationDifference=Math.abs(pos.SunElevation-pos.SatelliteElevation);
   }

pos.SunSatElDifference=formatvalue(pos.ElevationDifference,5);
...
var SatelliteLongitudeMoon = MoonLonArray[0];
MonSatLon=MoonLonArray[1]*1;
//alert(MonSatLon);

if (SatelliteLongitudeMoon=="Not Available") 
  {
  SatelliteElevationMoon="N/A";
  ...
  //ElevationDifference=Math.abs(document.Angles.MoonGeoEl.value-document.Angles.SatelliteElevationMoon.value);
   ...
   {
   SatelliteElevationMoon=formatvalue(Elevation2(MonSatLon,SiteLat,SiteLon,0),5);
   ElevationDifference=Math.abs(MoonEl-SatelliteElevationMoon);
   }

var MoonSatElDifference = formatvalue(ElevationDifference,5);
//
